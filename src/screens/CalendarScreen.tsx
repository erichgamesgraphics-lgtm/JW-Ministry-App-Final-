...
export const CalendarScreen: React.FC<CalendarScreenProps> = ({
  ...
  return (
    <div className="space-y-4 pb-24 max-w-lg mx-auto">
      {/* Screen Title & Google Calendar Sync Badge */}
      <div className="pt-2 flex items-center justify-between">
        ...
        <button
          ...
          disabled={isSyncing}
          className="flex items-center gap-1.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#131D31] px-3 py-1.5 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all shadow-xs cursor-pointer disabled:opacity-50"
          title={settings.googleCalendarConnected ? 'Google Calendar Connected' : 'Click to sync with Google Calendar'}
        >
          {isSyncing ? (
            <RefreshCw className="h-3.5 w-3.5 animate-spin text-blue-600 dark:text-blue-400" />
          ) : (
            <Cloud className={`h-3.5 w-3.5 ${settings.googleCalendarConnected ? 'text-blue-600 dark:text-blue-400' : 'text-slate-400'}`} />
          )}
          <span className="text-[11px]">
            {isSyncing ? 'Syncing...' : settings.googleCalendarConnected ? 'GCal Synced' : 'Sync GCal'}
          </span>
        </button>
      </div>

      {syncFeedback && (
        <div className="rounded-2xl bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800 p-3 text-xs font-medium text-blue-700 dark:text-blue-300">
          {syncFeedback}
        </div>
      )}
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        {ev.googleCalendarEventId && (
                          <div className="inline-flex items-center gap-1 text-[11px] font-semibold text-blue-600 dark:text-blue-400">
